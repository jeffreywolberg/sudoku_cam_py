...
X_train = add_black(X_train[:50], padding=int((im_length-28)/2))
# X_test = add_black(X_test[:50], padding=int((im_length-28)/2))
...
for i in range(10, 13):
    X_train[i] = cv2.threshold(X_train[i], 120, 255, cv2.CV_8U)[1].astype('float32')
peek_at_data(X_train, [10,11,12,3,4,5,6,7,8])
quit()

# top, bottom = find_top_and_bottom_limits(X_train[0])
# left, right = find_left_and_right_limits(X_train[0])
# print(top, bottom, left, right)
# pct_height_occupied, pct_width_occupied = (bottom-top) / im_length, (right-left) / im_length
# print(pct_height_occupied, pct_width_occupied)
# peek_at_data(X_train, [0])
...
adam = tensorflow.keras.optimizers.Adam(learning_rate=1e-4)
